...
export default async function AdminTarefasPage() {
  ...
  const { data: usuarios } = await supabase.from('usuarios').select('*').eq('perfil', 'usuario')
  const { data: tarefas } = await supabase
        .from('tarefas').select('*, usuario:usuarios!usuario_id(*)').order('data_limite', { ascending: true })
